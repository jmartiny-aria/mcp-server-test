...
async def read_resource(uri: str) -> str:
    ...
    if uri == "weather://current":
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get("https://api.open-meteo.com/v1/forecast?latitude=40.7128&longitude=-74.0060&current_weather=true")
    ...
    if name == "get_weather":
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                geo_response = await client.get(f"https://geocoding-api.open-meteo.com/v1/search?name={city}")
                ...
                if geo_data.get("results"):
                    ...
                    return [TextContent(
                        type="text",
                        text=f"🌤️ Weather in {city}, {country}\n\n"
                    ...
                    return [TextContent(type="text", text=f"❌ City '{city}' not found. Please check the spelling.")]
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                return [TextContent(
                    type="text", 
                    text=f"🤓 Random Fact:\n\n{fact_data.get('text', 'Did you know? Octopuses have three hearts!')}"
                )]
            except Exception as e:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if "error" not in data:
                    ...
                    return [TextContent(
                        type="text",
                        text=f"🚀 NASA APOD - {data.get('date', 'Today')}\n\n"
                    ...
                    return [TextContent(type="text", text=f"❌ NASA API Error: {data.get('error', {}).get('message', 'Unknown error')}")]
            except Exception as e:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.get(f"https://musicbrainz.org/ws/2/artist/?query={artist_name}&fmt=json&limit=3")
                data = response.json()
                ...
                if data.get("artists"):
                    ...
                    return [TextContent(type="text", text=f"🔍 No artist found for '{artist_name}'. Try a different name or spelling.")]
            except Exception as e:
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if data.get("meals"):
                    ...
                    return [TextContent(type="text", text=f"🔍 No recipes found for '{query}'. Try searching for common dishes like 'pasta', 'chicken', 'cake', etc.")]
            except Exception as e:
        ...
        async with httpx.AsyncClient() as client:
            try:
                response = await client.get(f"https://openlibrary.org/search.json?q={query}&limit={limit}")
                data = response.json()
                ...
                if data.get("docs"):
                    ...
                    return [TextContent(type="text", text=f"📚 No books found for '{query}'. Try different keywords or author names.")]
            except Exception as e:
